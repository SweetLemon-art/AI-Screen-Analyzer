...
interface AiProvider {
    ...
    suspend fun analyze(bitmap: Bitmap, context: AnalysisContext, settings: CaptureSettings = CaptureSettings.DEFAULT, userPrompt: String? = null): AnalysisResult
}

enum class AiProviderType { GEMINI, LOCAL }

class GeminiAiProvider(private val delegate: VisionAnalyzer) : AiProvider {
    override val type: AiProviderType = AiProviderType.GEMINI
    override suspend fun analyze(bitmap: Bitmap, context: AnalysisContext, settings: CaptureSettings, userPrompt: String?): AnalysisResult {
        return delegate.analyze(bitmap, context.withUserPrompt(userPrompt), settings)
    }
}
...
class LocalAiScreenProvider(
    ...
    override suspend fun analyze(bitmap: Bitmap, context: AnalysisContext, settings: CaptureSettings, userPrompt: String?): AnalysisResult {
        ...
        return try {
            ...
            val prompt = buildPrompt(context.withUserPrompt(userPrompt))
            var rawResponse = ""
            ...
            val completed = withTimeoutOrNull(generationTimeoutMs) {
                delegate.generate(prompt, imageBytes).collect { event ->
                    when (event) {
                        ...
                        is LocalAiEvent.Failed -> generationFailure = event.error
                    }
                }
                true
            } ?: false
            ...
            generationFailure?.let {
                return failure(context, startTime, it.message ?: "Local AI generation failed")
            }
            if (!completed) {
    ...
    private fun parseResponse(context: AnalysisContext, startTime: Long, rawResponse: String): AnalysisResult {
        ...
        return try {
            ...
            AnalysisResult(
                ...
                observations = json.optJSONArray("observations")?.toStringList().orEmpty(),
    ...
    private companion object { const val DEFAULT_GENERATION_TIMEOUT_MS = 120_000L }
}

class AiProviderRouter(providers: List<AiProvider>, initialProvider: AiProviderType = AiProviderType.GEMINI) {
    ...
    init {
        ...
        require(providersByType.containsKey(initialProvider)) { "Initial AI provider is not registered: $initialProvider" }
    }

    fun select(type: AiProviderType): Result<Unit> = if (providersByType.containsKey(type)) {
        selected = type
        Result.success(Unit)
    } else {
        Result.failure(IllegalArgumentException("AI provider is not registered: $type"))
    }
